...
function prepare_file_info {
  version_on_file=$(cut -d " " -f 1 $file_path)
...
function set_release_notes {
  release_notes=$(git log $latest_tag..HEAD --merges --pretty=tformat:"%h %s <br/>")
  # Checking if the release notes are empty to get individual commits instead
  if [ -z $release_notes ]; then release_notes=$(git log $latest_tag..HEAD --pretty=tformat:"%h %s <br/>"); fi
...
function validate_version_file {
  if [ -z "$VERSION_FILE_NAME" ]; then echo "VERSION_FILE_NAME is missing from your configuration"; exit $?; fi
  file_path=$(find . -name $VERSION_FILE_NAME)
  if [ -z "$file_path" ]; then echo "Cannot locate version file"; exit $?; fi
}
...
files=$(git diff --name-status $latest_tag HEAD | grep $VERSION_FILE_NAME)
